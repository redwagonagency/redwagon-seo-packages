...
export default function IntegrationConnectPanel({ ga4Connected, gscConnected }: Props) {
  ...
  async function connectGoogle(type: "ga4" | "gsc") {
    setBusy(type);
    await signIn("google", {
      callbackUrl: `/dashboard?autoConnect=${type}`,
    });
  }
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <button
          type="button"
          onClick={() => void connectGoogle("ga4")}
          disabled={ga4 || busy !== null}
        ...
        >
          ...
          <div className="text-xs text-slate-500 mt-1">
            {ga4 ? "✓ Connected" : busy === "ga4" ? "Redirecting to Google…" : "Authorize & connect GA4 →"}
          </div>
        </button>
        <button
          type="button"
          onClick={() => void connectGoogle("gsc")}
          disabled={gsc || busy !== null}
          className="rounded-xl border border-slate-200 px-4 py-3 text-left hover:border-[#f15b27] transition disabled:opacity-60"
        >
          <div className="flex items-center gap-2">
          ...
          <div className="text-xs text-slate-500 mt-1">
            {gsc ? "✓ Connected" : busy === "gsc" ? "Redirecting to Google…" : "Authorize & connect GSC →"}
          </div>
        </button>
      </div>
    </div>
