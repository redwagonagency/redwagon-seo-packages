...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { keyword, location = 2840, language = "en", limit = 120 } = body;
    ...
    const [ideasResult, trendsResult, serpResult] = await runWithApiUsageUserContext(userId, () =>
      Promise.allSettled([
        getKeywordIdeasLabs(seed, location, language, Math.min(Math.max(limit, 30), 300)),
    ...
    // Build trend direction map from phrase trends (last 8 periods)
    ...
    // Map SERP top URL for each idea keyword (fallback: seed-level top result)
    ...
    return Response.json(
      { error: error instanceof Error ? error.message : "Content ideas request failed" },
      { status: 500 }
    );
  }
}
