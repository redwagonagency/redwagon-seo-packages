...
export async function GET() {
  ...
  try {
    ...
    if (isPrismaMissingTableError(error, "IndustryStat")) {
      return NextResponse.json({ stats: [] });
    }
    throw error;
  }
...
export async function POST(req: NextRequest) {
  ...
  if (!industry || !metricKey || !Number.isFinite(metricValue)) {
    return NextResponse.json({ error: "industry, metricKey, metricValue are required" }, { status: 400 });
  }
  ...
  try {
    ...
    if (isPrismaMissingTableError(error, "IndustryStat")) {
      return NextResponse.json({ error: "IndustryStat table missing. Run prisma db push." }, { status: 500 });
    }
    throw error;
  }
}
